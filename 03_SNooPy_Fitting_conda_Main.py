...
DirSnoopyFiles = sys.argv[1]
# print DirSnoopyFiles

# What bands to fit, optical or optical+nir:
BandType = sys.argv[2]   # Options: (OpticalNIR, Optical)

# Fit specific observer-frame bands? For instance, fit BVR bands only?
FitSpecificBands = sys.argv[3] == 'True' # Options: (True, False)

# If FitSpecificBands = True, then specify the bands to fit.
if FitSpecificBands == True:

    SpecificBands = sys.argv[4]
...
kcorrData = sys.argv[5] == 'True' # Options: (True, False)
...
Use_k_stretch = sys.argv[6] == 'True' # (True, False)

# Python installation used to run SNooPy
# Options: (Conda, Canopy)
# If 'Conda' then I can set the values of the cosmological parameters to be
...
if debug:
    if PythonUsed == "Conda": print 'Ho = %s'%H0_user
...
textfile_1 = open(DirSaveOutput + 'Fitting_Settings_Results.txt', 'w')
...
for file in the_list:
    try:
        ...
        if FitSpecificBands == True: # Final fit all the data
            if debug: print "Bands to plot:", BandsToFit
            s.fit(BandsToFit, dokcorr=kcorrData, k_stretch=Use_k_stretch,
                reset_kcorrs=True)
    	    if debug: print 'Fitted bands:', BandsToFit

        elif FitSpecificBands == False:

            if BandType == 'Optical': # Final fit all the data
                if debug: print "Bands to plot:", OpticalBands
                s.fit(OpticalBands, dokcorr=kcorrData, k_stretch=Use_k_stretch,
                    reset_kcorrs=True)
                if debug: print "Fitting optical bands only: done"

            elif BandType == 'OpticalNIR': # Final fit all the data
                if debug: print "Bands to plot:", s.data.keys()
                s.fit(dokcorr=kcorrData, k_stretch=Use_k_stretch,
                    reset_kcorrs=True)
                if debug: print "Fitting optical+nir bands: done"
        ...
        if debug: print "Done: write supernova name to the text file."
        ...
        if debug: print "Done: Write match between observed to rest-frame bands."

        textfile_1.write(' \n')
        textfile_1.write('    z_CMB (NED) = %.5f  \n'%s.zcmb)
        if debug: print 'Done: Write z_CMB.'
        ...
        if debug: print 'Done: Write (EBVgal, e_EBVgal).'

        if PythonUsed == "Conda":

            textfile_1.write('    EBVhost = %.5f +/- %.5f  %.5f \n'%(
                s.EBVhost, s.e_EBVhost, s.systematics()["EBVhost"]))
            if debug: print "Done: EBVhost, e_EBVhost written."
            textfile_1.write('    Tmax = %.4f +/- %.4f  %.5f \n'%(
                s.Tmax, s.e_Tmax, s.systematics()["Tmax"] ))
            textfile_1.write('    dm15 = %.4f +/- %.4f  %.5f \n'%(
                s.dm15, s.e_dm15, s.systematics()["dm15"] ))
            textfile_1.write('    Distance mu = %.5f +/- %.5f  %.5f # Snoopy default \n'%(
                s.DM, s.e_DM, s.systematics()["DM"] ))

            textfile_1.write('    Distance mu = %.5f +/- %.5f  %.5f # LCDM\
(H0=%s, Om0=%s, Ode0=%s) \n'%(
                s.get_distmod(H0=H0_user, Om0=Om0_user, Ode0=Ode0_user), s.e_DM,
                s.systematics()["DM"], H0_user, Om0_user, Ode0_user))
            textfile_1.write('    Distance mu = %.5f +/- %.5f  %.5f # LCDM\
(H0=72.0, Om0=0.28, Ode0=0.72) \n'%(
                s.get_distmod(H0=72.0, Om0=0.28, Ode0=0.72), s.e_DM,
                s.systematics()["DM"]))
            textfile_1.write('    Distance mu = %.5f +/- %.5f  %.5f # LCDM\
(H0=73.24, Om0=0.28, Ode0=0.72) \n'%(
                s.get_distmod(H0=73.24, Om0=0.28, Ode0=0.72), s.e_DM,
                s.systematics()["DM"]))

        elif PythonUsed == "Canopy":
            textfile_1.write('    EBVhost = %.5f +/- %.5f  0.0 \n'%(
                s.EBVhost, s.e_EBVhost))
            if debug: print "EBVhost, e_EBVhost written."
            textfile_1.write('    Tmax = %.4f +/- %.4f  0.0 \n'%(
                s.Tmax, s.e_Tmax ))
            textfile_1.write('    dm15 = %.4f +/- %.4f  0.0 \n'%(
                s.dm15, s.e_dm15 ))
            textfile_1.write('    Distance mu = %.5f +/- %.5f  0.0 \n'%(
                s.DM, s.e_DM ))
        ...
        if debug: print "%s. Preparing to plot the fit."%s.name
        ...
        if debug: print 'y_loc = %s'%y_loc

        s.plot(epoch=True)

        plt.text(x_loc, y_loc, r"$\Delta$m15 = %.2f $\pm$ %.2f"%(s.dm15,s.e_dm15),
                     color='black', fontsize=10)
        plt.text(x_loc, y_loc+0.5, r"$z_{\rm hel}$ = %.3f" %(s.z),
                    color='black', fontsize=10)
        plt.text(x_loc, y_loc+1.0, r"$\mu$ = %.3f $\pm$ %.3f"%(s.DM,s.e_DM),
                    color='black', fontsize=10)
        plt.text(x_loc, y_loc+1.5, r"$T_{\rm Bmax}$ = %.2f $\pm$ %.3f"%(s.Tmax,s.e_Tmax),
                    color='black', fontsize=10)
        plt.text(x_loc, y_loc+2.0, r"E(B-V)$_{\rm MW}$ = %.3f $\pm$ %.4f "%(
                 s.EBVgal, s.e_EBVgal),color='black', fontsize=10)
        plt.text(x_loc, y_loc+2.5, r"E(B-V)$_{\rm host}$ = %.3f $\pm$ %.3f"%(
                    s.EBVhost,s.e_EBVhost),color='black', fontsize=10)
        plt.savefig(DirSaveOutput+NameDataFileToSave+"_PlotFitText.png",
                    format='png', dpi=100)
        # plt.savefig(DirSaveOutput+NameDataFileToSave+"_PlotFitText_high.png",
        #             format='png', dpi=300)
        plt.close()

        if debug: print "%s. Plot fit with text: done."%s.name
        if debug: print '%s. All Plotting done.'%s.name
        ...
        y_loc = s.get_max(bands=s.filter_order[0])[1]+2.5

        plt.text(-7, y_loc, r"$\Delta$m15 = %.2f $\pm$ %.2f"%(s.dm15,s.e_dm15),
                     color='black', fontsize=10)
        plt.text(-7, y_loc+0.2, r"$z_{\rm hel}$ = %.3f" %(s.z),
                    color='black', fontsize=10)
        plt.text(-7, y_loc+0.6, r"$\mu$ = %.3f $\pm$ %.3f"%(s.DM,s.e_DM),
                    color='black', fontsize=10)
        plt.text(-7, y_loc+0.4, r"E(B-V)$_{\rm host}$ = %.3f $\pm$ %.3f"%(
                    s.EBVhost,s.e_EBVhost), color='black', fontsize=10)
...
k-corrections."%(s.name, NumSim)
            ...
            print "%s. Simulation to determine k-corr uncertainties saved \
            to JSON files."%s.name
            """
...
fail_list.close();fail_list.close();
textfile_1.close();textfile_1.close();

print "   "
print "# -- %i SNe fitted and %i failed -- "%(countSN, countSNFail)
